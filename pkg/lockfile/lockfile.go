...
// Get opens a read-write lock file, creating it if necessary.  The Locker
// object it returns will be returned unlocked.
func Get(path string) (Locker, error) {
	lockfilesLock.Lock()
...
// GetRO opens a read-only lock file.  The Locker object it returns will be
// returned unlocked.
func GetRO(path string) (Locker, error) {
	lockfilesLock.Lock()
	defer lockfilesLock.Unlock()
	if lockfiles == nil {
		lockfiles = make(map[string]*lockfile)
