...
@test "manifests" {
	...
	digest3=${digest3// *}
	storage set-image-data -f ${TESTDIR}/random1 $image manifest-random1
	...
	[[ "$output" =~ "RepoTag: localhost/fooimage:latest" ]]
	[[ "$output" =~ "RepoDigest: localhost/fooimage@sha256:$digest1" ]]
	[[ "$output" =~ "RepoDigest: localhost/fooimage@sha256:$digest2" ]]

	run storage image localhost/fooimage@sha256:$digest1
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" =~ "ID: $image" ]]
	[[ "$output" =~ "Name: $name" ]]
